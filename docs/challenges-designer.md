...
- **灵感（v5.1）**：只看游玩时长 base = max(1, round(playTime/30))，最终 min(5, round(base × insp-up(+15%/级)))——单局至多 5 点，稀有度/隐藏不影响。上限动态：100 + 每款已精通桌游按稀有度 +1/+2/+3/+4（每款一次，转生后重新积累）。在线与离线自动游玩都入账。
- **立项**（-10 灵感）：名称（2~10 字）+ 类型（8 主题）+ 体量（小盒/标准/大盒，成本基数 30/100/300、牌套需求 50/150/300）。
...
- **需求按游戏日结算（v5.1，原逐秒）**：每过 1 个游戏日（24 秒）结算一次——基础关注 k = min(10, 随机 1~5 + floor(支持/100))，潜在购买人数 = k×10（× 事件流量乘区），每人独立判定（在线每 tick / 离线复用同一函数按天补结算，cap 沿用离线 1h）：
  1. 基础关注 k = min(10, 随机 1~5 + floor(支持/100))，潜在购买 = k×10；
